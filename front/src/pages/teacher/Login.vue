        ...
        <div class="w-full lg:w-1/2 p-8 lg:p-12 flex flex-col justify-center">
          <div class="max-w-sm mx-auto w-full">
            ...
            <div class="text-center mb-8">
              <h2 class="text-2xl font-bold text-gray-800 mb-2">教师登录</h2>
              <p class="text-gray-600">请输入您的工号和密码</p>
            </div>
...
const loginRules: FormRules = {
  username: [
    ...
    { min: 2, message: '工号长度不能少于2位', trigger: 'blur' }
  ],
  password: [
...
const hasSavedCredentials = computed(() => {
  return localStorage.getItem('teacherRememberMe') === 'true' && 
         localStorage.getItem('savedTeacherUsername')
})
  ...
  try {
    ...
    if (success) {
      ...
      if (rememberMe.value) {
        ...
        localStorage.setItem('savedTeacherUsername', loginForm.value.username)
        // 注意：在生产环境中，密码不应该以明文形式保存在localStorage中
        // 这里仅为演示功能，实际应用中应该使用更安全的方式
        localStorage.setItem('savedTeacherPassword', loginForm.value.password)
      } else {
        // 清除所有保存的登录信息
        localStorage.removeItem('teacherRememberMe')
        localStorage.removeItem('savedTeacherUsername')
        localStorage.removeItem('savedTeacherPassword')
      }
...
onMounted(() => {
  ...
  // 恢复记住的用户名和密码
  const savedRememberMe = localStorage.getItem('teacherRememberMe')
  const savedUsername = localStorage.getItem('savedTeacherUsername')
  const savedPassword = localStorage.getItem('savedTeacherPassword')
